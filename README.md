...
接收速度 ≈ 相机每秒成功解码的帧数 × 每帧字节数。网页版的 418 KB/s 纪录用的是定制 zxing 解码器；
本 App 走 ML Kit，实际吞吐取决于手机相机与对焦，一般远低于发送端上限。接收页实时显示
**速率（KB/s）** 和 **捕获率**（seq 跨度中真正解出来的比例），按它调发送端：

- **捕获率 < 50%**：降低发送端帧率（先试 30，再试 24），或把手机拿近、扶稳、避开反光；
- **捕获率 ≈ 100%**：还有余量，可提高发送端字节/帧（2331 → 2953）或帧率，直到捕获率开始掉；
- 传输完成后会显示用时与平均速率，便于横向对比不同设置。
